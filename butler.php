...
$result = $_db->query('SELECT id FROM butlers WHERE authcode = ?', array($request['authcode']));
$server = $result->fetch();
...
if (!empty($server['id'])) {
	...
	if ($request['request'] == "registerServer") {
		$newAuthcode = createId(16);
		$_db->query('UPDATE butlers SET remote_authcode = ? WHERE id = ?', array($newAuthcode, $server['id']));
		$response['result'] = "ok";
		$response['authcode'] = $newAuthcode;
